                    ...
                    || {
                        ComputePipelineBuilder::new(
                            Shader::new(include_str!("randomwalker_weights.glsl"))
                                ...
                                .define("ND", nd)
                                .define(weight_function.define_name(), 1),
                        )
                        .build(device)
                    },
                )?;
...
fn mean_filter<D: DynDimension>(
    ...
    let size = 2 * extent + 1;
    let kernel = super::array::from_vec(vec![1.0 / size as f32; size]);
    let kernels = Vector::fill_with_len(&kernel, t.dim().n());
    super::volume_gpu::separable_convolution(t, kernels)
}
...
fn variance(
    ...
    let uncorrected_variance =
        super::volume_gpu::mean(diff_sq.compile().unwrap().try_into().unwrap());
