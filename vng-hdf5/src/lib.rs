...
use std::rc::Rc;
use vng_core::array::VolumeEmbeddingData;
use vng_core::data::{Coordinate, CoordinateType};
use vng_core::dim::D3;
...
fn to_size_vector<C: CoordinateType>(
    value: Vec<hdf5::Ix>,
) -> Result<Vector<D3, Coordinate<C>>, vng_core::Error> {
    to_vector(value.into_iter().map(|v| v as u32).collect())
}

fn to_vector<I: Copy, O: From<I> + Copy>(value: Vec<I>) -> Result<Vector<D3, O>, vng_core::Error> {
    match *value {
        [z, y, x] => Ok([z, y, x].into()),
